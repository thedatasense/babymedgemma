...
from babygemma.data_index import PHENOMENA, build_index
from babygemma.paths import CACHE
...
def load_feature_cache(path=None):
    global _FEATS
    if _FEATS is None:
        path = path or os.environ.get("NANO_FEATS") or os.path.join(CACHE, "medsiglip_feats.pt")
...
def load_pooled_cache(path=None):
    ...
    global _POOLED
    if _POOLED is None:
        path = path or os.environ.get("NANO_POOLED") or os.path.join(CACHE, "medsiglip_pooled.pt")
...
def build_tokenizer(index=None, max_len=None):
    """MedGemma's SentencePiece tokenizer, pruned to the pieces this corpus uses."""
    from babygemma.tokenizer import build_gemma_tokenizer
    index = index or build_index()
    return build_gemma_tokenizer(index, max_len=max_len or int(os.environ.get("NANO_MAXLEN", "20")))
...
class NanoDataset(Dataset):
    def __init__(self, examples: list[Ex], tok, feats=None, pooled=None):
