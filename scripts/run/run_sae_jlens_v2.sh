...
CUDA_VISIBLE_DEVICES=0 python scripts/experiments/sae.py --arch gemma --grounding-token --layer 1 --model-steps 8000 \
  --out "$PWD/results_transfer/sae" 2>&1 | grep -vE "use_fast|slow image|Warning" &
echo "=== Jacobian lens on the scaled grounded model ===" 
CUDA_VISIBLE_DEVICES=1 python scripts/experiments/jlens.py --arch gemma --grounding-token --model-steps 8000 \
