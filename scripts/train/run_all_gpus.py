...
def build_jobs():
    ...
    # scripts moved into scripts/<role>/; resolve each job's script to its real path
    SCRIPT_PATH = {
        "train.py": os.path.join(HERE, "scripts", "train", "train.py"),
        "experiment_a.py": os.path.join(HERE, "scripts", "experiments", "experiment_a.py"),
        "experiment_e.py": os.path.join(HERE, "scripts", "experiments", "experiment_e.py"),
    }

    def tj(tag, outdir, script="train.py", result="result.json", **flags):
        args = [sys.executable, SCRIPT_PATH[script], "--steps", str(STEPS)]
        ...
        if os.environ.get("NANO_GROUND") and script in ("train.py", "experiment_a.py", "experiment_e.py"):
